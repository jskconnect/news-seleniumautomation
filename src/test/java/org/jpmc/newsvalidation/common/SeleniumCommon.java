...
public class SeleniumCommon {
	...
	public void searchElementsInPage(String strsearchnews, String strtosplit) throws Exception {
		log.info("validate the news to search appears in the google search results");		
		List<WebElement> elementlist = Driver.getDriverInstance().findElements(By.xpath("//*[contains(text(),'" + strsearchnews + "')]"));
		Assert.assertTrue("Validate the NewtoSearch Text appears in google search result:", elementlist.size()>0);
	...
	public void readPageSource(String strsearchnews) throws Exception {
		if (Driver.getDriverInstance().getPageSource().contains(strsearchnews)) {
			...
			Assert.assertTrue(Driver.getDriverInstance().getPageSource().contains(strsearchnews));
		} else {
